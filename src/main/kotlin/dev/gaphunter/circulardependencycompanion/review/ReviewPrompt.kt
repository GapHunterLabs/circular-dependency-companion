package dev.gaphunter.circulardependencycompanion.review
 ...
 * number of explicit Refresh clicks in the tool window -- never on
 * install, never on a timer, and never for the passive initial scan
 * that runs automatically when the tool window first opens (only a
 * deliberate re-scan counts as real use). Counts invocations directly
...
object ReviewPrompt {
    ...
    /** How many explicit Refresh clicks before the prompt shows once. */
    private const val HITS_BEFORE_PROMPT = 5

    private const val KEY_HIT_COUNT = "dev.gaphunter.circulardependencycompanion.review.hitCount"
    private const val KEY_ANSWERED = "dev.gaphunter.circulardependencycompanion.review.answered"

    private const val NOTIFICATION_GROUP_ID = "Circular Dependency Companion"

    // Marketplace listing (real ID resolved after first publish).
    private const val MARKETPLACE_URL = "https://plugins.jetbrains.com/plugin/33675-circular-dependency-companion/reviews"

    /**
     * Call this once per real, explicit Refresh click. Safe to call on
     * any thread.
     */
